...
공유 투표는 로그인·연락처 권한 없이 동작합니다. 공유 딥링크는 앱 루트의 쿼리 파라미터를 사용해 정적 WebView에서도 열리며, 기존 하위 경로 링크도 계속 읽을 수 있습니다. 샌드박스/QR 테스트에서는 현재 배포 ID가 포함된 intoss-private:// 테스트 링크를 사용하고, 정식 출시 후에는 intoss://replaypick?... 공개 링크를 사용합니다. 앱인토스 문서상 intoss:// 링크는 정식 출시 전에는 동작하지 않으므로, 출시 전 링크 공유 테스트는 QR 테스트 환경에서 진행해야 합니다. 친구가 선택하면 토스 공유 시트가 열리고, 친구가 원작성자를 선택해 결과 링크를 보내면 원작성자 앱에서 질문과 선택된 답장을 확인할 수 있습니다.
